...
class JournalingUsageTrackerTest {
  ...
  @get:Rule var testSpoolDir = TemporaryFolder()
  ...
  fun trackerTimeoutTest() {
    ...
    try {
      ...
      // After the timeout there should be one spool file that is locked and one that is ready for
      // reading.
      ...
      // After close we expect two separate spool files, each containing one of the events.
      val afterClose = getSpoolDetails(testSpoolDir.root.toPath())
      assertEquals(0, afterClose.lockedFiles.size.toLong())
