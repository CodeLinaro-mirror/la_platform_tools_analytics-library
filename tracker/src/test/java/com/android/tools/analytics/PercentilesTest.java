...
public class PercentilesTest {
  ...
  private static Percentiles createGaussianEstimator(
      double[] targets, int numSamples, ArrayList<Double> samples) {
  ...
  public void smallEstimationMerge() {
    ...
    // This can be mitigated by allowing more raw samples to accumulate before forming the
    // estimation.
    // Here all the estimators will not be interpolated yet, so the merge is the same as building an
  ...
  public void subdistibutionsMerge() {
    ...
    // Note that in this case, the max in a -abs(normal) distribution will be < the min of an
