...
class CommonMetricsDataTest {
  ...
  fun getJvmArchitectureTest() {
    SystemPropertyOverrides().use { systemPropertyOverrides ->
      ...
      assertEquals(
        CpuArchitecture.UNKNOWN_CPU_ARCHITECTURE,
        CommonMetricsData.jvmArchitecture)
  ...
  fun getJvmDetailsTest() {
    ...
    HostData.runtimeBean = object : StubRuntimeMXBean() {
    ...
    try {
      ...
      val expectedNoOptions = JvmDetails.newBuilder()
        .setName(VM_NAME)
        .setVendor(VM_VENDOR)
      ...
      val expectedAllOptions = JvmDetails.newBuilder()
        .setName(VM_NAME)
        .setVendor(VM_VENDOR)
        .setVersion(VM_VERSION)
  ...
  fun getGarbageCollectionStatsTest() {
    try {
      ...
      HostData.garbageCollectorBeans = ImmutableList.of(
        StubGarbageCollectionBean.fixedValue(FIRST_GC, 100, 123),
        StubGarbageCollectionBean.fixedValue(SECOND_GC, 404, 512))

      val firstExpected = GarbageCollectionStats.newBuilder()
        .setName(FIRST_GC)
        .setGcCollections(100)
        .setGcTime(123)
        .build()

      val secondExpected = GarbageCollectionStats.newBuilder()
        .setName(SECOND_GC)
      ...
      val thirdExpected = GarbageCollectionStats.newBuilder()
        .setName(FIRST_GC)
        .setGcCollections(100)
        .setGcTime(111)
        .build()

      val fourthExpected = GarbageCollectionStats.newBuilder()
        .setName(SECOND_GC)
        .setGcCollections(97)
        .setGcTime(512)
        .build()
  ...
  fun getJavaProcessStatsTest() {
    try {
      ...
      HostData.memoryBean = object : StubMemoryBean() {
      ...
      HostData.classLoadingBean = object : StubClassLoadingBean() {
        override fun getLoadedClassCount(): Int {
          return 100
        }
      }

      HostData.threadBean = object : StubThreadBean() {
        override fun getThreadCount(): Int {
          return 5
        }
      }

      val expected = JavaProcessStats.newBuilder()
