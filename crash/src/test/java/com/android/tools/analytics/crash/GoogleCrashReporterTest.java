...
public class GoogleCrashReporterTest {
  // Most of the tests do a future.get(), but since they are uploaded to a local server, they should
  // complete relatively quickly.
  // This rule enforces a shorter timeout for the tests. If you are debugging, you probably want to
  // comment this out.
  @Rule public TestRule myTimeout = Timeout.seconds(15);
  ...
  public void setup() throws Exception {
    ...
    myReporter =
        new GoogleCrashReporter(
            "http://localhost:" + port + "/submit",
            UploadRateLimiter.create(infiniteQps),
            true,
            false);
  }
  ...
  public void checkParameterOverriding() throws Exception {
    myTestServer.setResponseSupplier(
        httpRequest ->
            new DefaultFullHttpResponse(
                HttpVersion.HTTP_1_1, HttpResponseStatus.OK, httpRequest.content()));
    ...
    assertTrue(
        "Request should contain overridden osName. Full request body: " + response,
        response.contains(
  ...
  public void checkRateLimiting() {
    UploadRateLimiter mockLimiter =
        mock(UploadRateLimiter.class); // defaults to a rate limiter that denies all requests
    ...
    try {
      ...
      Truth.assertThat(e.getCause().getMessage())
          .isEqualTo("Exceeded Quota of crashes that can be reported");
    }
  }
  ...
  public void checkBinaryBodyAddedForLongValue() {
    ...
    verify(mockBuilder)
        .addTextBody(
            key,
            Ascii.truncate(value, maxBytesForValue, truncationIndicator),
            ContentType.DEFAULT_TEXT);
    verify(mockBuilder)
        .addBinaryBody(key + "-full", value.getBytes(), ContentType.DEFAULT_TEXT, key + ".txt");
  }
